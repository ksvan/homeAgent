...
async def _instrument_process_tool_call(
    ...
    """Pass-through process_tool_call that emits control events for tools MCP calls."""
...
def get_mcp_server() -> MCPServerStreamableHTTP | None:
    """Return the running Tools MCP server instance, or None if not configured."""
    return _mcp_server


def _create_mcp_server() -> MCPServerStreamableHTTP | None:
    settings = get_settings()
    if not settings.tools_mcp_url:
        logger.info("Tools MCP not configured — execution tools disabled")
        return None
    return MCPServerStreamableHTTP(
        url=settings.tools_mcp_url,
...
async def start_mcp() -> MCPServerStreamableHTTP | None:
    """Connect to the Tools MCP server. Called during FastAPI lifespan startup.
    ...
    for attempt in range(1, _MCP_MAX_RETRIES + 1):
        try:
            ...
            _mcp_server = server
            logger.info("Tools MCP connection established (%s)", get_settings().tools_mcp_url)
            return server
            ...
            if attempt < _MCP_MAX_RETRIES:
                logger.warning(
                    "Tools MCP not reachable (attempt %d/%d: %s) — retrying in %ds",
                ...
                logger.warning(
                    "Tools MCP not reachable after %d attempts — execution tools disabled",
                    _MCP_MAX_RETRIES,
                )
    return None
...
async def stop_mcp() -> None:
    """Disconnect from the Tools MCP server. Called during FastAPI lifespan shutdown."""
    global _mcp_server
    if _mcp_server is not None:
        try:
            await _mcp_server.__aexit__(None, None, None)
        except Exception:
            logger.warning("Error during Tools MCP shutdown", exc_info=True)
        finally:
            _mcp_server = None
            logger.info("Tools MCP connection closed")
